...
class Data:
    ...
    def sort(self):
        ...
        if 'AEM: (AWI + MOSAIC + N-ICE2015)' in self.name:
            print('ENTERED')
            ind = [i for name, i in zip(self.name, range(len(self.name))) if name == 'AEM: (AWI + MOSAIC + N-ICE2015)'][0]
            order.remove(int(ind))  # Remove index of AEM
            order.append(int(ind))  # Append AEM index to the end
...
directory = "/dmidata/users/ilo/projects/RRDPp/satellite/Final_files/collocated_files/filtration_test"
