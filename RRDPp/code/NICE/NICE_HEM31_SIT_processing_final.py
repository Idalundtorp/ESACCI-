...
ofile = f'ESACCIplus-SEAICE-RRDP2+-SIT-{name}-HEM-p2_v2.nc'
...
files = glob.glob(f'/dmidata/users/ilo/projects/RRDPp/RawData/{name}/HEM31/N-ICE2015_EM31_v1/*.txt')
...
for ifile in files:
    ...
    if 'corr_xres' in data.dtype.names: # not geographical coordinates (e.g. not lat/lon)
        continue
    elif 'lat' in data.dtype.names:
        latitude = data['lat']
        longitude = data['lon']
        SIT = data['thick']
        # no time provided
        date0 = dt.datetime.strptime(file[:8] + 'T00:00:00',"%Y%m%dT%H:%M:%S")
        # add a second for each observation so that gridding processing is running
        t = [date0 + dt.timedelta(seconds=i) for i in range(len(SIT))]
        dataOut.pp_flag = np.array([2]*len(SIT))
    elif 'corr_latres' in data.dtype.names:
        latitude = data['corr_latres']
        longitude = data['corr_lonres']
        SIT = data['thickres']
        # date is provided as year, month, day + seconds since start of day
        gps_date = data['gpstimeres']
        t = [dt.datetime.fromordinal(int(gd)) + dt.timedelta(days=gd%1) - dt.timedelta(days=366) for gd in gps_date]
        dataOut.pp_flag = np.array([0]*len(SIT))
    elif 'latres' in data.dtype.names:
        latitude = data['latres']
        longitude = data['lonres']
        SIT = data['thickres']
        # no time provided
        date0 = dt.datetime.strptime(file[:8] + 'T00:00:00',"%Y%m%dT%H:%M:%S")
        # add a second for each observation so that gridding processing is running
